...
import { Send, FileText, CheckCircle, BarChart3, Users, Loader2 } from 'lucide-react'
...
export default function AdminBlog() {
  ...
  async function handlePost(e: React.FormEvent) {
    ...
    try {
      ...
      setSuccessMessage('Artigo publicado com sucesso e sincronizado no banco.')
      setTitulo('')
      setSlug('')
      setResumo('')
      setConteudo('')
        ...
        <div className="max-w-4xl mx-auto">
          <header className="mb-8">
            ...
            <p className="text-sm text-muted-foreground mt-1">
              Escreva e publique artigos técnicos sincronizados com a API.
            </p>
          </header>

          {successMessage && (
            <div className="mb-6 bg-emerald-500/10 text-emerald-600 border border-emerald-500/20 p-4 rounded-xl flex items-center gap-3 animate-fade-in">
          ...
          <form onSubmit={handlePost} className="space-y-6 bg-card border border-border p-8 rounded-2xl shadow-sm">
            <div className="grid gap-6 md:grid-cols-2">
              <div>
                ...
                <input
                  ...
                  className="w-full border border-border bg-transparent p-3 rounded-xl outline-none focus:border-accent transition-colors text-foreground"
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full border border-border bg-muted/50 p-3 rounded-xl outline-none text-muted-foreground cursor-not-allowed"
                  disabled
                  placeholder="redes-lte-privadas-na-engenharia"
                />
              </div>
            </div>
            ...
            <div>
              ...
              <input
                ...
                className="w-full border border-border bg-transparent p-3 rounded-xl outline-none focus:border-accent transition-colors text-foreground"
                placeholder="Uma breve descrição para a listagem do blog..."
              />
            </div>

            <div>
              <label className="block text-sm font-semibold text-primary mb-2">Imagem de capa (URL)</label>
              <input
                type="url"
                value={imagemCapa}
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full border border-border bg-transparent p-3 rounded-xl outline-none focus:border-accent transition-colors text-foreground resize-none font-sans leading-relaxed"
                placeholder="Desenvolva o texto técnico detalhado aqui..."
              />
            </div>
            ...
            <div className="flex items-center justify-between pt-4 border-t border-border">
              ...
              <button
                type="submit"
                disabled={isLoading}
                className="bg-accent hover:bg-brand-blue disabled:opacity-50 text-white font-bold px-6 py-3 rounded-xl inline-flex items-center gap-2 transition-colors cursor-pointer shadow-md"
              >
                {isLoading ? (
                  <>
